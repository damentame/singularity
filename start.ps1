...
if (Test-Path $FrontendEnv) {
    ...
    $null = Test-EnvVar $feVars "VITE_SUPABASE_URL"      "your local Supabase API URL - run: npx supabase start"
    $null = Test-EnvVar $feVars "VITE_SUPABASE_ANON_KEY"  "anon key from ``npx supabase start`` output"
} else {
    ...
    Write-Hint "Then run:  npx supabase start"
    Write-Hint "and paste the API URL + anon key it prints into frontend\.env:"
    Write-Hint "  VITE_SUPABASE_URL=http://127.0.0.1:54321"
    Write-Hint "  VITE_SUPABASE_ANON_KEY=<anon key from the command output>"
    Write-Hint ""
...
if (Test-Path $BackendEnv) {
    ...
    $null = Test-EnvVar $beVars "SUPABASE_URL"              "your local Supabase API URL - run: npx supabase start"
    $null = Test-EnvVar $beVars "SUPABASE_ANON_KEY"          "anon key from ``npx supabase start`` output"
    $null = Test-EnvVar $beVars "SUPABASE_SERVICE_ROLE_KEY"   "service_role key from ``npx supabase start`` output (keep this secret!)"
} else {
    Write-Fail "backend\.env  not found"
    Write-Hint "Create it from the example:"
    Write-Hint "  Copy-Item backend\.env.example backend\.env"
    Write-Hint "Then run:  npx supabase start"
    Write-Hint "and paste the API URL, anon key, and service_role key it prints:"
